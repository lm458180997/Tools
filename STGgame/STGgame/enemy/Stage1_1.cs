...
namespace stg.enemy
{
    public class Stage1_1 : Enemy
    {
        private RectangleF myrect1 = new RectangleF(0, 0, 32, 32);
        private RectangleF myrect2 = new RectangleF(128, 0, 32, 32);
        private RectangleF myrect3 = new RectangleF(352, 0, 32, 32);
        float distance;
        float fly_time;                           //出现时的飞行时间
        float Add_speed;                          //进入时的加速度
        float Add_speed2;                         //离开时的加速度
        public Stage1_1(float x,float y)
       {
            this.coordinate.X = x;
            this.coordinate.Y = y;
            this.radius = 20;
            ...
            fly_time = 3;
            this.speed = 2 * distance / fly_time;       //初速度
            this.Add_speed = speed / fly_time;          //加速度
            this.Add_speed2 = Add_speed / 3;
            this.direction = getdirection();            //方向指向player 
            this.HP = 20;
       }
